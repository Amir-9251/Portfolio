...
export function ProjectCard({ title, description, image, tags, link, github }: ProjectCardProps) {
  return (
    <motion.div whileHover={{ scale: 1.02 }} transition={{ duration: 0.3 }} className="h-full" layout>
      <Card className="overflow-hidden border-0 h-full transition-all duration-300 bg-card/30 backdrop-blur-sm flex flex-col shadow-md hover:shadow-lg">
        ...
        <CardContent className="p-6 space-y-4 flex flex-col flex-1">
          ...
          <div className="flex gap-4 pt-2">
            {link && (
              <Button variant="outline" size="sm" asChild className="rounded-full gradient-border">
